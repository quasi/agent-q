...
```json
{
  ...
  "description": "Disk persistence operations for sessions",
  "properties": {
    "save_session": {
      "type": "object",
      "properties": {
        "session_id": {"type": "string", "pattern": "^session-[0-9]{8}-[0-9]{6}-[A-F0-9]{4}$"},
        "filepath": {"type": "string", "description": "Path where session was saved"}
      },
...
|-------|-------|----------|
| `FILE-ERROR` | Cannot write to sessions directory | Check permissions; create directory if missing |
| `SERIALIZATION-ERROR` | Session data contains unserializable objects | Sanitize data before save; log problematic objects |
| `DESERIALIZATION-ERROR` | Corrupted or invalid session file | Skip corrupted file; log error; offer recovery |
| `SESSION-NOT-FOUND` | Requested session doesn't exist on disk | Return nil; suggest creating new session |
| `VERSION-MISMATCH` | Loading session with unsupported version | Attempt migration; warn user if migration fails |
| `DISK-FULL` | No space available for session save | Alert user; suggest cleanup; fail gracefully |
| `PERMISSION-DENIED` | Cannot access sessions directory | Alert user; suggest permission fix |

**Prevention:**
- Ensure sessions directory exists and is writable at startup
- Validate session data before serialization
- Use defensive file I/O with error handling

**Handling:**
- All save/load operations wrapped in error handlers
- Errors logged to `*Messages*` buffer
- User notified via minibuffer messages
- Failed operations return nil; system remains stable

**Error Recovery:**
- Corrupted files: Skip and continue with other sessions
- Missing directory: Auto-create with mode 0700 (owner-only access)
- Serialization failure: Save fallback minimal session

**Recovery target:** All recoverable errors handled within a single save/load operation (no retry loops). Corrupted sessions skip in < 100ms.
