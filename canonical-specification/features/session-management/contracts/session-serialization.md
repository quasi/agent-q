...
```json
{
  ...
  "description": "Serialization format for session persistence",
  "properties": {
    "version": {
      "type": "integer",
      "enum": [1, 2],
    ...
    "created_at": {"type": "integer", "description": "Universal Time (v2) or Emacs time (v1)"},
    ...
    "messages": {
      ...
      "items": {
        ...
        "properties": {
          "role": {
            "oneOf": [
              {"type": "string", "enum": [":user", ":assistant", ":system", ":debug"]},
              {"type": "string", "enum": ["user", "assistant", "system", "debug"]}
            ]
          },
          "content": {"type": "string"},
...
|-------|-------|----------|
| `UNSUPPORTED-VERSION` | Session file has unknown version | Reject load; suggest manual inspection |
| `MALFORMED-PLIST` | Invalid plist structure in session file | Attempt partial recovery: skip individual malformed message entries while preserving valid ones. If > 50% of entries are corrupt, discard the entire session and log a warning. |
| `TIMESTAMP-CONVERSION-ERROR` | Cannot convert Elisp time to Universal Time | Use current time; log original value |
| `SYMBOL-ROLE-ERROR` | v1 role is symbol, not keyword | Convert to keyword; warn about migration |
| `MESSAGE-ORDER-ERROR` | Messages in unexpected order | Re-sort chronologically; validate |
| `ENCODING-ERROR` | Non-UTF-8 characters in content | Replace with replacement character; log |

**Prevention:**
- Version check first thing during deserialization
- Validate plist structure before accessing slots
- Defensive timestamp conversion with fallbacks
- Character encoding checks on load

**Handling:**
- Version errors: Hard fail with clear message
- Malformed data: Skip bad entries, load rest
- Timestamp errors: Replace with sensible default
- Encoding errors: Sanitize and continue

**Migration:**
```lisp
(defun migrate-v1-to-v2 (v1-plist)
  "Safely migrate v1 session to v2 format."
  (handler-case
      (convert-v1-to-v2 v1-plist)
    (error (e)
      (warn "Migration failed: ~A" e)
      nil)))  ; Return nil on failure
```

**Compatibility:**
- v1 sessions: Automatically migrated on load
- v2 sessions: Native format, no conversion
- Future versions: Must provide migration path
