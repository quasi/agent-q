...
2. Uses markdown headers and code fences that LLMs can parse as structured sections
...
**Recommendation:** Map context types to language tags: :code → `` ```lisp ``, :error → `` ```text ``, :file → auto-detect from extension, :text → `` ```markdown ``
